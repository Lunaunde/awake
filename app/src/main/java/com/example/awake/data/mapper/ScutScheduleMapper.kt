...
class ScutScheduleMapper {
    fun map(payload: ScutSchedulePayload, timetableId: Long): MappedSchedule {
        ...
        payload.courses.forEach { dto ->
            ...
            val parsedWeeks = WeekExpressionParser.parse(dto.weeks)
            period.warning?.let(warnings::add)
            parsedWeeks.warning?.let(warnings::add)
            if (dto.day !in 1..7 || period.warning != null || parsedWeeks.weeks.isEmpty()) return@forEach
    ...
    private fun ScutCourseDto.remoteKey(): String = sha256(listOf(source, name, teacher, room, day, periods, weeks, className).joinToString("|"))
    private fun sha256(value: String): String = MessageDigest.getInstance("SHA-256").digest(value.toByteArray()).joinToString("") { "%02x".format(it) }
    private fun colorFor(index: Int): Int = listOf(0xff4f6bed, 0xff0f9d78, 0xffd97706, 0xffc24170, 0xff7c3aed)[index % 5].toInt()
}
